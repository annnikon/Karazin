...
var Locale = $H({
  format:     (typeof Date.CultureInfo == 'undefined' ? '%d %b %Y' : Date.CultureInfo.formatPatterns.shortDate),
  monthNames: (typeof Date.CultureInfo == 'undefined' ? $w('Січень Лютий Березень Квітень Травень Червень Липень Серпень Вересень Жовтень Листопад Грудень') : Date.CultureInfo.monthNames),
  dayNames:   (typeof Date.CultureInfo == 'undefined' ? $w('Неділя Понеділок Вівторок Середа Четвер Пятниця Субота') : Date.CultureInfo.dayNames),
...
var Timeframe = Class.create({
  ...
  initialize: function(element, options) {
    ...
    this.buttons = $H({
      ...
      today:    $H({ label: 'С',      element: $(this.options.get('todayButton')) }),
  ...
  validateRange: function(start, end) {
    ...
    window.myfirstday=start;
     window.mylastday=end;
     window.IsDateSelected=true;
  ...
  clearRange: function() {
    ...
    if (this.options.keys().include('onClear')) this.options.get('onClear')();
      window.myfirstday=null;
    window.mylastday=null;
    window.IsDateSelected=false;
  },
